...
if uploaded_file is not None:
    ...
    if df is not None:
        ...
        if st.button('Calcular Necesidades de TDPs'):
            ...
            total_tdp_required = 0
            total_share_target = sum(value_share_targets.values())
            for brand, tdp in predictions.items():
                if tdp is not None:
                    st.write(f"{brand}: {tdp:.2f} TDPs necesarios")
                    total_tdp_required += tdp
                else:
                    st.write(f"{brand}: Predicción no disponible")
            
            st.write(f"**Total Share Target**: {total_share_target}")
            st.write(f"**Total TDPs necesarios**: {total_tdp_required:.2f}")
